...
class DefensiveMCTAgent(MCTCaptureAgent):
  ...
  def registerInitialState(self, gameState):
    ...
    # Initial chaseDest list, which is used to chase the food that is the last one been eatten
    self.chaseDest = []
  ...
  def chooseAction(self, gameState):
    ...
    """
    #  if didn't detect any invader and didn't find any food has been 
    #  eaten, then go to initial defender position. If return two actions 
    #  with the same values, random choose one action as the best action.
    ...
    #  "Scared! Attack Now!" 
    #  Go to the enemy territory to eat food when our agents become "scared" 
    ...
    #   Go back home when the "scared" time left less than 10 steps 
    #   Give the priority to chasing invader  
      if len(currInvader) > 0:
        foodLeft = len(self.getFood(gameState).asList())
        # If find invader, chase invader  
        self.chaseDest = []
      elif len(self.chaseDest)>0:
        #  "chaseFood" is several actions return by AStar
        ...
        #  If reach the eaten food destination, empty the list
          self.chaseDest = []
   
      elif len(self.detectDest)>0:
        # Use A Star to go to the detect destination
  ...
  def Heuristic(self,loc,destination):
    # Use manhattanDistance to calculate the heuristic distance 
